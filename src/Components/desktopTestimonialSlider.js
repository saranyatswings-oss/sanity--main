// Desktop-only testimonial slider initializer.
// Exports `initTestimonialSlider()` which starts the slider and returns a cleanup function.
...
export default function initTestimonialSlider() {
  ...
  if (typeof gsap === 'undefined') {
    // If gsap wasn't bundled for some reason, warn and noop.
    // HomePage already imports gsap so this is unlikely, but safe-guard.
    // eslint-disable-next-line no-console
    console.warn('Wings testimonials (desktop): GSAP not found.');
    return () => {};
  }

  const wrapper = document.querySelector('.homepage-testimonial__cards-wrapper');
  if (!wrapper) return () => {};

  const pages = Array.from(wrapper.querySelectorAll('.homepage-testimonial__cards'));
  ...
  function getCards(page) {
    return Array.from(
      page.querySelectorAll('.homepage-testimonial__card-left, .homepage-testimonial__card-right'),
    );
  }
  ...
  function getParts(card) {
    ...
    const texts = Array.from(
      card.querySelectorAll('.elementor-image-box-content, .homepage-testimonial__card-content'),
    );
    return { img, texts };
  }
  ...
  function animateTo(nextIndex, direction = 1) {
    ...
    getCards(curPage).forEach((card, idx) => {
      ...
      if (img)
        tl.to(img, { clipPath: exitClip, duration: 0.95, ease: 'power2.inOut' }, at);
      tl.to(texts, { opacity: 0, duration: 0.75, ease: 'power1.in' }, 0);
    });
